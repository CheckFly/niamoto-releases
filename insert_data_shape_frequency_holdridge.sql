with 
	hodlridge_classif as (
		select * from (
			select gid_emprise,mnt_object,'Très sec' classe, sum(pixelcount) pixelcount from atlas_pn.pn_emprises_holdridge
...
INSERT INTO niamoto_portal.data_shape_frequency(class_object, class_name, class_value, shape_id)
	SELECT concat('holdridge_', hc.mnt_object) class_object,
		hc.classe, 
		Case when hc.pixelcount >0 then round((hc.pixelcount::float/sap.allpixel)::numeric,2) else 0 end  class_value,
