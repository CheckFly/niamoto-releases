...
WITH 
	shape_emprises AS (SELECT gid, name FROM niamoto_preprocess.emprises),
	data_elevation_land AS (SELECT gid_emprise, class_elevation, sum(pixelcount) pixelcount FROM niamoto_preprocess.emprises_raster WHERE mnt_object='land' GROUP BY gid_emprise, class_elevation),
	data_elevation_forest AS (SELECT gid_emprise, class_elevation, sum(pixelcount) pixelcount FROM niamoto_preprocess.emprises_raster WHERE mnt_object='forest' GROUP BY gid_emprise, class_elevation),

	data_elevation_landum AS (SELECT gid_emprise, class_elevation, sum(pixelcount) pixelcount FROM niamoto_preprocess.emprises_raster WHERE mnt_object='land_um' GROUP BY gid_emprise, class_elevation),
	data_elevation_forestum AS (SELECT gid_emprise, class_elevation, sum(pixelcount) pixelcount FROM niamoto_preprocess.emprises_raster WHERE mnt_object='forest_um' GROUP BY gid_emprise, class_elevation),

	data_elevation_forest_core AS (SELECT gid_emprise, class_elevation, sum(pixelcount) pixelcount FROM niamoto_preprocess.emprises_raster WHERE mnt_object='forest_core' GROUP BY gid_emprise, class_elevation),
	data_elevation_forest_mature AS (SELECT gid_emprise, class_elevation, sum(pixelcount) pixelcount FROM niamoto_preprocess.emprises_raster WHERE mnt_object='forest_mature' GROUP BY gid_emprise, class_elevation),

	grid_elevation AS (SELECT gid,name, series FROM generate_series(100,1700,100) series cross join shape_emprises),
	area_land_emprise AS (SELECT gid, land_area_ha, land_um_area_ha FROM niamoto_preprocess.emprises),
	ratio_emprise AS (SELECT gid, CASE WHEN sum_land>0 then land_area_ha/sum_land ELSE 0 END r_land, CASE WHEN sum_landum>0 then land_um_area_ha/sum_landum ELSE 0 END r_landum FROM niamoto_preprocess.emprises 
				LEFT JOIN 
				...
				FROM niamoto_preprocess.emprises_raster GROUP BY gid_emprise) a
				ON gid=gid_emprise
			 )
...
SELECT 	a.gid shape_id, 
	...
	series as class_name, 
	CASE WHEN c.pixelcount>0 THEN round(h.pixelcount::numeric/c.pixelcount::numeric,5) ELSE 0 END as class_value
FROM
	grid_elevation a 
LEFT JOIN 
...
SELECT 
	...
	series as class_name, 
	CASE WHEN c.pixelcount>0 THEN round(i.pixelcount::numeric/c.pixelcount::numeric,5) ELSE 0 END as class_value
FROM
	grid_elevation a 
LEFT JOIN 
	data_elevation_forest c ON a.gid=c.gid_emprise AND a.series=c.class_elevation
...
SELECT 
	...
	series as class_name, 
	CASE WHEN c.pixelcount>0 THEN round((c.pixelcount-h.pixelcount -i.pixelcount)::numeric/c.pixelcount::numeric,5) ELSE 0 END class_value
FROM
	grid_elevation a 
LEFT JOIN 
	data_elevation_forest c ON a.gid=c.gid_emprise AND a.series=c.class_elevation
...
ALTER FUNCTION niamoto_portal.insert_shape_frequency_elevation()
    OWNER TO amapiac;
